...
好的，现在A的数据终于可以百分百确定交给B了。但是现在B拿到一堆数据帧，啥也看不懂，不知道我想表达的是什么意思。于是我在数据帧的某一部分告诉他，我用的是HTTP协议，你就用HTTP协议解析就行了。于是他用浏览器一解析，芜湖，最后我想告诉他的内容就显示出来了：“你真可爱”。这就是**应用层**。
